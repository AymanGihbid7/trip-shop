/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { VoyagesWhereInput } from "./VoyagesWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { VoyagesOrderByInput } from "./VoyagesOrderByInput";

@ArgsType()
class VoyagesFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => VoyagesWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => VoyagesWhereInput, { nullable: true })
  @Type(() => VoyagesWhereInput)
  where?: VoyagesWhereInput;

  @ApiProperty({
    required: false,
    type: [VoyagesOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [VoyagesOrderByInput], { nullable: true })
  @Type(() => VoyagesOrderByInput)
  orderBy?: Array<VoyagesOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { VoyagesFindManyArgs as VoyagesFindManyArgs };
